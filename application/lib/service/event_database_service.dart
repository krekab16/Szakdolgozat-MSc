...
class EventDatabaseService {
  ...
  Future<void> addRating(String userId, String eventId, double rating) async {
    try {
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        Map<String, dynamic> ratings = snapshot.get('ratings') ?? {};
  ...
  Future<double> getRating(String userId, String eventId) async {
    try {
      ...
      Map<String, dynamic> ratings = snapshot.get('ratings') ?? {};
      return ratings.containsKey(eventId) ? ratings[eventId] as double : 0.0;
    } on FirebaseException catch (e) {
      throw Exception(e.message);
    }
  }
  ...
  Future<List<EventDTO>> fetchRatedEvent(String userId) async {
    try {
      ...
      final Map<String, dynamic>? data =
      snapshot.data() as Map<String, dynamic>?;
      final Map<String, dynamic> ratings = data?['ratings'] as Map<String, dynamic>;
      final List<String> ratedEvents = ratings.entries
          .where((entry) => entry.value == 5.0)
          .map((entry) => entry.key)
          .toList();
      final QuerySnapshot querySnapshot = await _firestore
          .collection('events')
          .where(FieldPath.documentId, whereIn: ratedEvents)
          .get();
      final List<EventDTO> events =
      await Future.wait(querySnapshot.docs.map((doc) async {
        final data = doc.data() as Map<String, dynamic>;
